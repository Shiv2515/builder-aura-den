          ...
          .map((pair: any, index: number) => {
            ...
            return {
              ...
              aiScore: Math.min(100, Math.max(15,
                25 + Math.floor((pair.volume?.h24 || 0) / 100000) + // Volume factor (0-20 points)
                (Math.abs(pair.priceChange?.h24 || 0) > 15 ? 15 : 0) + // Volatility bonus (0-15 points)
                (((pair.txns?.h24?.buys || 0) + (pair.txns?.h24?.sells || 0)) > 200 ? 10 : 0) + // Activity bonus (0-10 points)
                ((pair.liquidity?.usd || 0) > 150000 ? 20 : (pair.liquidity?.usd || 0) > 50000 ? 10 : 0) + // Liquidity bonus (0-20 points)
                (pair.pairCreatedAt && (Date.now() - pair.pairCreatedAt) > 7 * 24 * 60 * 60 * 1000 ? 10 : 0) // Age bonus (0-10 points)
